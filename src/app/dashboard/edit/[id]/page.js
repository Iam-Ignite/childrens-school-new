...
export default function Page() {
  ...
  const fetchEvent = async (eventId) => {
    setIsLoading(true);
    try {
      const response = await axios.get(`/api/event/?id=${eventId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataObj.append("eventId", "678c6a71ad3c05910ecec87f");
      formDataObj.append("headline", formData.headline);
      formDataObj.append("subHeadline", formData.subHeadline);

      if (selectedFile) {
        formDataObj.append("imageUrl", selectedFile);
      }

      const response = await axios.put("/api/event", formDataObj, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (response.data.success) {
        // Fetch the updated event data to update the UI
        const updatedEvent = response.data.data; // Ensure backend sends updated event data
        setPreviewUrl(updatedEvent.imageUrl); // Update preview URL with the updated image URL
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 p-6">
        ...
        <form
          ...
          className=" mx-auto bg-white shadow-md rounded-lg p-6"
        >
          {error && (
          ...
          <div className="mb-4">
            ...
            <div className="mt-2 flex flex-col items-center w-1/2 gap-1 py-10 border rounded-lg p-4 bg-gray-100">
              {previewUrl ? (
                ...
                <p className="mt-2 text-sm text-gray-600">Click add image</p>
              )}
